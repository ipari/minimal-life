---
title: "유선 헤드폰"
lang: ko
kind: item
translation_key: headphones
permalink: /ko/items/headphones/
alternate_url: /en/items/headphones/
thumbnail: /assets/images/products/headphones.png
model: "Audio-Technica ATH-M50x"
status: "사용 중"
category: "전자기기"
item_order: 40
acquired_date: 2023-02-08
---

## 구매했던 이유

충전 없이 오래 사용할 청취 도구가 필요했다.

## 이 모델을 선택한 이유

충전이 필요 없는 유선 방식이고 케이블을 교체할 수 있어 오래 사용하기 좋았다. 접어서 보관할 수 있고 수리용 부품을 구하기 쉬운 점도 선택에 영향을 주었다.

## 현재 상태

한쪽에서만 소리가 난다. 케이블 교체로 수리 가능한지 점검 중이다.

## 참고 링크

[제품 정보 보기](https://example.com/headphones)
